...
module.exports = {
    async execute(member,selectGuildWelcomer,selectedChannel,add) {
        ...
        if (add==true) {
            context.fillText(lang.get(guild.lang).canvas["join"], canvas.width / 2.5, canvas.height / 3.5);
        }
        else {
            context.fillText(lang.get(guild.lang).canvas["leave"], canvas.width / 2.5, canvas.height / 3.5);
        }
