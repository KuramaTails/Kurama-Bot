...
module.exports = {
	async execute(modal,lang) {
        ...
        updateEmbed.fields[0].name = lang.get(modal.guild.lang).update.leaver["text"]+` \`${textLeave}\``
        await selectedMessage.edit({embeds:[updateEmbed]})
        await modal.editReply({ content: lang.get(modal.guild.lang).update.leaver["text"] +` ${textLeave}.`, ephemeral: true })
