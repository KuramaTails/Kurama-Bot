...
module.exports = {
	...
	async execute(interaction,player,lang,voiceChannel) {   
        ...
        function optQueue(voiceChannel) {
            ...
            queueEmbed.setTitle(lang.get(interaction.guild.settings.lang).player.embeds["queue"])
            queue.forEach(song => {
                if (i<25) {
                    var index = i > 0 ? i : lang.get(interaction.guild.settings.lang).player.embeds["currentlyPlaying"];
