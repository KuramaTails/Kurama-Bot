module.exports = {
	async execute(modal,player,lang) {
        if (!modal.guild.settings.playerPlugin.channelId) return
        var link = modal.getTextInputValue('textinput-customid')
        var voicechannel = modal.member.voice.channel
        if (!voicechannel) {
            await modal.editReply({ content: lang.get(modal.guild.settings.lang).commands.player.commands.errors["memberJoin"], ephemeral: true })
            return
        }
        ...
        var string = lang.get(modal.guild.settings.lang).commands.player.commands["play"]
