...
module.exports = {
	...
	async execute(queue) {
        ...
        .setColor('#0099ff')
        .setTitle(bot.lang.get(queue.clientMember.guild.settings.lang).commands.player.embeds["playing"]+`: \`${playlist[0].name}\``)
        .setThumbnail(`${playlist[0].thumbnail}`)
        .setURL(`${playlist[0].url}`)
        .setDescription(bot.lang.get(queue.clientMember.guild.settings.lang).commands.player.embeds["duration"]+`: \`${playlist[0].formattedDuration}\`\n`)
        ...
        moreButton.addComponents(
            ...
            .setLabel(bot.lang.get(queue.clientMember.guild.settings.lang).buttons.buttons["btnMoreCommand"]+"🔽")
            .setStyle(`SECONDARY`),);
